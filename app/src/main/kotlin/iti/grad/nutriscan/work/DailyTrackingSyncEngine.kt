...
class DailyTrackingSyncEngine(
    ...
    /** Syncs [yesterday]'s daily-tracking and retries any pending meal push/delete.
     * Returns true only if everything succeeded (worker should retry otherwise). */
    suspend fun sync(yesterday: LocalDate): Boolean {
        ...
        for (entry in foodLogDao.getPendingSyncEntries()) {
